...
exports.generateVideoUrl = exports.deleteCourse = exports.getAdminAllCourses = exports.addReplyToReview = exports.addReview = exports.addAnswer = exports.addQuestion = exports.deleteCourseFromFeatured = exports.addCourseToFeatured = exports.getFeaturedCourses = exports.getCourseByUser = exports.getAllCourses = exports.getSingleCourse = exports.editCourse = exports.uploadCourse = void 0;
...
//function to get data of featured courses
exports.getFeaturedCourses = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const featuredCourses = await featured_model_1.default.findOne();
        res.status(200).json({
            success: true,
            featuredCourses,
        });
...
// Function to add a course to the featured list
exports.addCourseToFeatured = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        // Check if the course is already featured
        const featured = await featured_model_1.default.findOne({ featuredCourses: courseId });
        if (featured) {
            return res.status(400).json({ success: false, message: "Course already featured" });
        }
        // Add courseId to the featuredCourses list
        await featured_model_1.default.findOneAndUpdate({}, { $push: { featuredCourses: courseId } }, { upsert: true });
        // Return success response
        res.status(200).json({ success: true, message: "Course added to featured list successfully" });
    }
    catch (error) {
        // Handle errors
        return next(new ErrorHandler_1.default(error.message, 500));
...
// Function to delete a course from the featured list
exports.deleteCourseFromFeatured = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const { courseId } = req.body;
        // Verify if courseId exists
        const course = await course_model_1.default.findById(courseId);
        ...
        const featured = await featured_model_1.default.findOne({ featuredCourses: courseId });
        if (!featured) {
            return res.status(400).json({ success: false, message: "Course is not featured" });
        }
        // Delete courseId from the featuredCourses list
        await featured_model_1.default.findOneAndUpdate({}, { $pull: { featuredCourses: courseId } });
        // Return success response
        res.status(200).json({ success: true, message: "Course removed from featured list successfully" });
    }
    catch (error) {
        // Handle errors
        return next(new ErrorHandler_1.default(error.message, 500));
...
exports.addQuestion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const courseContent = course?.courseData?.find((items) => items._id.equals(contentId));
        if (!courseContent) {
            return next(new ErrorHandler_1.default("Invalid content id", 400));
        }
...
exports.addAnswer = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const question = courseContent?.questions?.find((items) => items._id.equals(questionId));
        if (!question) {
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        //if courseId does not exist, return error
        if (!courseExists) {
            return next(new ErrorHandler_1.default("You are not enrolled in this course", 404));
        }
        const course = await course_model_1.default.findById(courseId);
